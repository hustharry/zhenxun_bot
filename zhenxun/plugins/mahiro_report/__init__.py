...
_matcher = on_alconna(Alconna("真寻日报"), priority=4, block=True, rule=to_me())

_reset_matcher = on_alconna(
    Alconna("重置真寻日报"), priority=4, block=True, permission=SUPERUSER
)
...
async def _():
    message = MessageUtils.build_message("小叶子带大家下午来摸鱼！")
    await broadcast_group(message, log_cmd="真寻日报")
